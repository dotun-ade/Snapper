...
4. **Stays current**
   Runs twice daily (e.g. 5am and 6pm UTC). Every run is a **full run**: Snapper reads the entire Leads tab, sends it to Gemini in batches, synthesises a fresh ICP analysis, and **fully overwrites** the Google Doc. No incremental/delta mode — each run produces a complete, up-to-date document.

**Gemini request budget:** 5 calls per run (4 batch + 1 synthesis). Two runs per day = 10 calls, under the 20/day limit.
...
| `STATE_DIR` | Optional. Directory for `state.json` (default: project root). Set to a Railway volume path (e.g. `/data`) so state persists across deploys. |
...
The 20 requests per day limit is the binding constraint. Each full run uses 5 requests (4 batch + 1 synthesis); running twice daily uses 10, leaving headroom for retries or manual runs.
...
Snapper stores minimal run state in `state.json` (project root, or `STATE_DIR` if set, e.g. a Railway volume):
...
3. Add a **Volume** and set mount path to e.g. `/data`; set env `STATE_DIR=/data` so state persists across deploys.
4. Set the **Start Command** to `npm start`. Each time the service starts (including on **redeploy**), it runs one full analysis and exits. No need for a separate “run on deploy” step — starting the service is the run.
5. Add **Cron** triggers for twice-daily full runs, e.g.:
   - `0 5 * * *` (5am UTC)
   - `0 18 * * *` (6pm UTC)
6. Deploy. You get a run on every redeploy plus two scheduled runs per day. Every run does a full CRM read and overwrites the ICP doc.
