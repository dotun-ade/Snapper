...
async function synthesize(batchSummaries, totalRows, runDate, usage) {
  const prompt = [
    ...
    `   Concrete ICP archetypes for Anchor based on the data, each with:`,
    `   - a short label (e.g. \"Regional Fintech Issuer\"),`,
    `   - typical country/region, industry, tier (Integrating+ / Live),`,
    `   - common product mix, deal size band, and use-case themes,`,
    `   - what tends to make them a strong fit (or weak fit) for Anchor.`,
    `   Include as many distinct profiles as you can confidently support from the data.`,
    `   The profiles must be grounded in the observed patterns above — no pure speculation or invented segments.`,
    ``,
